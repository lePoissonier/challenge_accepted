...
from elements import Pos


# when comments are inside classes i normalized which type of comments to you use
...
class Arena:
    ...
    def render(self):
        ...
        for row in self.board:
            for pos in row:
                if pos.knight:
                    print('KING ' + pos.knight.id, end='')
                elif len(pos.items):
                    print('X ' + pos.items[0].name[0] if pos.items[0] else '', end='')
                else:
                    print('  ', end='')
            print('')
        print('')
